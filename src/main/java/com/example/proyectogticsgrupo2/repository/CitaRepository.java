...
public interface CitaRepository extends JpaRepository<Cita, String> {
    ...
    @Query(value = "SELECT * FROM cita c inner join doctor d on (d.id_doctor=c.doctor_id_doctor) inner join paciente p on (p.id_paciente=c.paciente_id_paciente) WHERE doctor_id_doctor='10304011' and lower(concat(p.nombre,' ',p.apellidos)) like %?1%",
            nativeQuery = true)
